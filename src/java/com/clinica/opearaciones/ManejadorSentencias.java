package com.clinica.opearaciones;
...
//        return String.join(",", getNombreCampos());
        ...
        String auxi = "";
        for(int i=0; i<getNombreCampos().size(); i++){
            auxi += getNombreCampos().get(i);
            if(i != getNombreCampos().size()-1){
                auxi += ",";
            }
        }
        return auxi;  
